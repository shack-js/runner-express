...
export default async (apiFolder = '', urlPath = '', extension = '') => {
  ...
  const tmpModule = await getModule(tmp + extension)
  // console.log(tmpModule, { tmp, extension })
  if (!tmpModule) throw 'illegal urlPath'
  let authMethods = await getAuthMethods(tmp, extension, apiFolder)
...
async function getAuthMethods(modluePath = '', extension = '', baseFolder = 0) {
  let folder = dirname(modluePath), tarr = [], t = folder
  while (true) {
    if (authCache[t]) {
      return setCache(authCache[t].concat(tarr))
    }
    let m = await getModule(join(t, AUTH_FILE) + extension)
    if (m) tarr.unshift(m.default)
